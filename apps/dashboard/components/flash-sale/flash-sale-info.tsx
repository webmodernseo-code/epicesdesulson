  ...
  return (
    <div className="bg-white rounded-2xl  border border-gray-500/20">
      ...
      <div className="p-6">
        ...
        {/* Discount / Start Date / End Date Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {discountCards.map((card) => (
            <div key={card.label} className={`${card.bg} rounded-xl p-6`}>
              <p
                className={`text-xs  text-light-secondary-text font-medium mb-1`}
              >
                {card.label}
              </p>
        ...
        {/* Stats Row */}
        <div className="grid grid-cols-2 bg-gray-100 p-6 rounded-2xl md:grid-cols-4 gap-6">
          {statsItems.map((stat) => (
            <div key={stat.label} className="flex items-center gap-3">
              <div
                className={`size-10 rounded-full bg-warning-lighter flex items-center justify-center shrink-0`}
              >
