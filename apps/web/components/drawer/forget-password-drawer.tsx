...
interface ForgetPasswordDrawerProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function ForgetPasswordDrawer({
  isOpen,
  onClose,
}: ForgetPasswordDrawerProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          key="forget-password-drawer"
          initial={{ x: 200, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          exit={{ x: 200, opacity: 0 }}
          transition={{ duration: 0.25, ease: "easeInOut" }}
          className="forgot-password-page-sidebar fixed xl:top-[30px] xl:right-[22px] right-0 top-0 xl:h-[calc(100vh-52px)] h-full z-99 max-w-[600px] w-full bg-white xl:rounded-2xl rounded-none shadow-dark-z-24"
        >
          <div className="forgot-password-page-sidebar-header px-6 pt-6 pb-4 border-b border-gray-300 relative">
          ...
          <div className="forgot-password-page-sidebar-content p-10 flex flex-col gap-y-10 overflow-y-auto max-h-[calc(100%-70px)]">
